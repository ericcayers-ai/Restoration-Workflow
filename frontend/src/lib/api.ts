/*
 * REST client. Every call is a relative `/api/...` path ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â in dev the Vite
 * proxy forwards it to `restore serve`; in production the same build is
 * served *by* that backend, same origin (ARCHITECTURE.md section 1) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this
...
async function unwrap<T>(response: Response): Promise<T> {
  ...
  try {
    ...
    // Non-JSON error body ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â keep the status-text fallback above.
  }
...
export function health(): Promise<{ version: string; api_version: string; plugin_errors: string[] } | null> {
